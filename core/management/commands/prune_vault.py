...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ── 3. Clean empty directories in media/vault ──
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING(f"\n[DRY RUN] {pruned_count} file(s) ({mb_freed} MB) would be pruned from disk."))
        else:
            self.stdout.write(self.style.SUCCESS(f"\nSuccessfully pruned {pruned_count} expired evidence file(s) ({mb_freed} MB freed)."))
